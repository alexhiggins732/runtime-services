    ...
    public class GenericCallFactory
    {
        ...
        public class ResolverFactoryTest
        {
            public static void Test()
            {
                ...
                CtorHelper<bool>.SetResolver(() => default);
                ...
                var defaultFunc = new FuncDefault<int>();
                var fn = (IFunc)defaultFunc;
                ...
                Func<int, bool> fnBool2 = (i) => FnBool(i);

                void aa() => Console.WriteLine("do something");
                void aa2(FuncDefault<int> cb) => Console.WriteLine($"do something{fnBool(cb.Invoke().Result)}");
                void aa3<Ta>(FuncDefault<Ta> cb) => Console.WriteLine($"do something{cb.Invoke().Result}");
                Mbox(aa);
            ...
            private static void Mbox(Action aa)
            {
                throw new NotImplementedException();
            }


            //[Obsolete(@"Call has been replaced with Default<T>.Call(). Additionally IFunc<T>, IFuncObject interfaces 
            //    indicate Invoke returns a Func<> as a variable when it can be invoked later when it is actually a baked in method."
            //)]
            public struct FuncDefault<T> : IFunc<T>, IFunc
            {
                public static Func<T> fn = Generic<T>.Default;
                public IFuncResult<T> Invoke() => new FuncResult<T>(fn());
        ...
        public interface IFunc<T1, T2> : IFunc { new IFuncResult<T2> Invoke(); }
        public interface IFunc<T1, T2, T3> : IFunc { new IFuncResult<T3> Invoke(); }
        ...
        {
            public static IFuncResult Call(IFunc func) => (IFuncResult)func.Invoke();
